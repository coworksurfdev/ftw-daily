...
import {
  dateFromLocalToAPI, minutesBetween, nightsBetween, daysBetween
} from '../../util/dates'
...
export class CheckoutPageComponent extends Component {
  ...
  handlePaymentIntent(handlePaymentParams) {
    ...
    const fnHandleCardPayment = (fnParams) => {
      ...
      const {
        stripe, card, billingDetails, paymentIntent
      } = handlePaymentParams
  ...
  handleSubmit(values) {
    ...
    const {
      history, speculatedTransaction, currentUser, paymentIntent, dispatch
    } = this.props
    const {
      card, message, paymentMethod, formValues
    } = values
    const {
      name,
      addressLine1,
